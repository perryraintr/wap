...
app.controller("qrcode_cafecomment", function($scope, $http) {
	...
	if($scope.oid == undefined || $scope.oid == 0) {
		$scope.getMoney = false;
	} else {
		$scope.getMoney = true;
	}
...
//	$scope.oid = 2063;
	...
	$scope.sendMessage = function() {
		...
		var getOid = "";
		if($scope.getMoney) {
			getOid = "&oid=" + $scope.oid;
		}

		$http.get(getHeadUrl() + "store_comment_add.a?sid=" + $scope.id + getOid + "&mid=" + $scope.member.guid + "&star=" + $scope.star + "&m=" + $scope.message).success(function(response) {
