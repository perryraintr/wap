...
app.controller('product_detail_share', function($scope, $http) {
	$http.get("http://www.pinshe.org/v1/product.a?pid=" + GetQueryInt("pid")).success(
		function(response) {
			...
			$scope.hitCount(response.body.product_name + response.body.product_guid + "-share");
		}
	);
	...
	$scope.add = function(name, guid) {
		$scope.hitCount(name + guid + "-shareBuy");
	}
	// 统计数量
